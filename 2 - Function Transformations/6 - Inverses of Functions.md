...
1. Swapping the independent and dependent variable gives x = 3g(x) + 1, then rearranging gives g(x) = \frac{x - 1}{3}.
2. The function involves multiplying the dependent variable by 3, then adding 1. The inverse is subtracting by 1 then dividing by 3, giving g(x) = \frac{x - 1}{3}.
3. Graphing f then reflecting it over the line y = x results in a y-intercept of \frac{-1}{3} and a slope of \frac{x}{3}, so the inverse is g(x) = \frac{x}{3} - \frac{1}{3}, which is equivalent to g(x) = \frac{x - 1}{3}.
