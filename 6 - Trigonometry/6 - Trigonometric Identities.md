...
The reciprocal identities are the following: sin(\theta) = 1/csc(\theta), cos(\theta) = 1/sec(\theta), and tan(\theta) = 1/cot(\theta).
...
If we substitute sin(\theta) = OPP/HYP and cos(\theta) = ADJ/HYP from SOHCAHTOA into sin(\theta)/cos(\theta), we get sin(\theta)/cos(\theta) = (OPP/HYP)/(ADJ/HYP) = (OPP/HYP) \times (HYP/ADJ) = OPP/ADJ = tan(\theta). This gives us the quotient identity sin(\theta)/cos(\theta) = tan(\theta).